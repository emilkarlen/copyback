...
readonly sUsage='[-gkvV] {<testcase file>|-}'
...
readonly wslTestCommands='env env-cmds programs stop-after-parse stop-after-setup print-check-compare-eq-flags'

readonly VERSION='simptestcase 2.1.1'

readonly EXIT_SUCCESS=0
readonly EXIT_FAIL=1
readonly EXIT_INVOC=2
...
readonly STDOUT_RE='\<stdout\>'
readonly STDERR_RE='\<stderr\>'
readonly DEREFERENCE_RE='\<dereference\>'
readonly DEFAULT_RE='\<default\>'
readonly FILE_CONTENTS_RE='\<file-contents\>'
readonly DIR_CONTENTS_RE='\<dir-contents\>'
readonly TYPE_RE='\<type\>'
readonly MTIME_RE='\<mtime\>'
readonly MODE_RE='\<mode\>'
readonly ENV_UNSET_VAR_RE='^unset ([^=]+)$'
readonly ENV_VAR_RE_RE='^([^=]+)='
readonly FLAGS_RE='^(/([^/]*))?$'
readonly MK_ABS_PATH_RE='\<mkabspath\>'
readonly EXISTS_RE='\<exists\>'
readonly PREPEND_HOME_RE="\<prepend-home\>"
readonly KEY_RE='^([^ /]+)(/([^/]*))?$'
readonly NON_HOME_RE='\<non-home\>'
readonly FST_NON_HOME_RE="\<1-non-home\>"
readonly DIGITS_RE='^[0-9]+$'
readonly REPLACED_RE='\<replaced\>'
readonly ONLY_SPACE_RE='^[[:blank:]]*$'
...
{
    local sMsg1="preprocessing:PWD=${PWD}"
    local sMsg2="preprocessing:+${*}"
    if [ "${bVerb}" ]; then
	echo ${sMsg1}
	echo ${sMsg2}
    fi
    echo ${sMsg1} >> "${fileLoggParseTc}"
    echo ${sMsg2} >> "${fileLoggParseTc}"
}
...
# - showMsgStepFinish -
#
#   Skriver ut utdata för sista steget i testet.
...
#   3 <generated file>
#   4 [<file with correct contents>] (if check-type = file)
#
...
###############################################################################
function checkCompareEq_fileContents
{
    local fileSrc="$1"
    local fileDst="$2"
    # A #
	...
	# B2 #
	if ! diff "${fileSrc}" "${fileDst}" \
	    >> "${fileLoggOut}" 2>> "${fileLoggErr}" ; then
	    showMsgStepFinish 'FAIL' 'contents differ'
	    exit ${EXIT_FAIL}
	fi
    elif [ -d "${fileSrc}" ]; then
	# C #
...
#    B.1. Fail if <dst> is not a file.
#    B.2. Fail if <src> and <dst> differ in contents.
# C. Set 'deref' to the flag for dereferencing for `find'.
# D. Compare the directory-contents. Produce a listing of the files of each
#    directory using 'find' and 'sort', and then compare the listings using
#    'diff'. (Include the type of files in the listings so that different
#    types of files differe even if they got the same name.)
###############################################################################
function checkCompareEq_dirContents
{
    local fileSrc="$1"
    local fileDst="$2"
    # A #
    ...
    # C #
    if [[ "${lFlags}" =~ ${DEREFERENCE_RE} ]]; then
	local deref='-L'
    else
	local deref=''
    fi
    # D #
    find $deref "${fileSrc}" -printf "%P:%y\n" | sort > ../data/dir-contents-src.txt
    find $deref "${fileDst}" -printf "%P:%y\n" | sort > ../data/dir-contents-dst.txt
    if ! diff ../data/dir-contents-src.txt ../data/dir-contents-dst.txt \
	>> "${fileLoggOut}" 2>> "${fileLoggErr}" ; then
...
function checkCompareEq # <src> <dst> <flags>
{
    ...
    # C #
    if [ "${lFlags}" == '/' ] || [[ "${lFlags}" =~ ${DEFAULT_RE} ]]; then
	[ "${lFlags}" == '/' ] && lFlags=''
	if [ -f "${fileSrc}" ]; then
    ...
    # E #
    if [[ "${lFlags}" =~ ${FILE_CONTENTS_RE} ]]; then
	checkCompareEq_fileContents "${fileSrc}" "${fileDst}"
    fi
    # F #
    if [[ "${lFlags}" =~ ${DIR_CONTENTS_RE} ]]; then
	checkCompareEq_dirContents "${fileSrc}" "${fileDst}"
    fi
    # G #
    if [[ "${lFlags}" =~ ${DEREFERENCE_RE} ]]; then
	local deref='-L'
    ...
    # H #
    if [[ "${lFlags}" =~ ${TYPE_RE} ]]; then
	showMsgStepBegin 'check-compare-eq/type'
	local typeSrc=$(stat $deref --printf='%F' "${fileSrc}")
	local typeDst=$(stat $deref --printf='%F' "${fileDst}")
    ...
    # I #
    if [[ "${lFlags}" =~ ${MTIME_RE} ]]; then
	showMsgStepBegin 'check-compare-eq/mtime'
	local timeSrc=$(stat $deref --printf='%y' "${fileSrc}")
	local timeDst=$(stat $deref --printf='%y' "${fileDst}")
    ...
    # J #
    if [[ "${lFlags}" =~ ${MODE_RE} ]]; then
	showMsgStepBegin 'check-compare-eq/mode'
	local modeSrc=$(stat $deref --printf='%a' "${fileSrc}")
	local modeDst=$(stat $deref --printf='%a' "${fileDst}")
...
function testCommandEnv
{
	...
	if [[ "${aEnvCmds[$i]}" =~ ${ENV_UNSET_VAR_RE} ]]; then
	    echo ${BASH_REMATCH[1]}
	elif [[ "${aEnvCmds[$i]}" =~ ${ENV_VAR_RE_RE} ]]; then
	    echo ${BASH_REMATCH[1]}
	fi
    done |
    sort | uniq |
...
function parseProgram # <arrprefix> <pgmtype> <arg1> <arg2> <lFlags> <nNumCmdParts>
{
	...
	cmdline)
		...
		showMsgParseErrNotProgram "cmdline should have 1 argument"
		return $EXIT_PARSE
	    fi
	    eval ${cmdSetType}'cmdline'
	    if [[ "$lFlags" =~ ${PREPEND_HOME_RE} ]]; then
...
# A2 #
	...
	case "${aCmdParts[0]}" in
		...
		fileSrc=${aCmdParts[1]}
		fileDst=${aCmdParts[2]}
		if [[ ! "${lFlags}" =~ ${FST_NON_HOME_RE} ]]; then
...
#
#
# A. Spara aktuell katalog så vi kan byta tillbaka till den, byt sen till
#    testets rotkatalog.
...
#    F3. Spara exitkoden.
# G. Utför 'check.exitcode'.
# H. Utför 'check.prepare'.
# I. Utför 'check.{stdout,stderr}'.
...
# K. Utför 'check.compare.eq'.
# L. Utför 'check.custom'.
# M. Utför 'cleanup'.
# N. Testet har lyckats. Avsluta utan att skriva ut något (förr skrevs
...
# I #
	...
	if [[ "${check_channel__lFlags[${idx}]}" =~ ${REPLACED_RE} ]]; then
...
(
	...
	[[ "${sLine}" =~ ${ONLY_SPACE_RE} ]] || echo "${sLine}"
    done
)
...
#    2. Run m4.
#    3. Pop back directory.
#    4. Update 'filePrevious'.
# C. Strip comments.
...
function preprocessTestCase # <test case file>
{
    ...
    if [ "_${1}" == '_-' ]; then
	showPreprocessStepStart cp /dev/stdin ${filePrevious}
	cp /dev/stdin ${filePrevious} 2>> ${fileLoggParseTc}
	[ $? != 0 ] && preprocessError 'cp'
    else
	showPreprocessStepStart cp ${fileTc} ${filePrevious}
    ...
    showPreprocessStepStart stripCommentsAndEmptyLines '<' ${filePrevious} '>' ${fileTcPostUncomment}
    ...
    showPreprocessStepStart cp ${filePrevious} ${fileTcFinal}
...
#
# D. Parse the test case and execute the env commands.
#   Läs testfallsfilen och utför testkommandot "env-cmds" om det ska göras.
#   D.0. Preprocess the testcase file if --no-preproc has not been given.
#        Otherwise, set fileTcFinal to the input file.
#   D.1. Läs testfallsfilen, avsluta om inläsningen misslyckades.
#   D.2. Utför testkommandona 'env-cmds' eller 'programs' om något av dem
#        angivits.
# E. Kontrollera att testfallet innehåller tillräcklig information för att
#    utgöra ett test. Avsluta om det inte gör det.
# F. Utför testet.
# G. Testet klart. Avsluta.
###############################################################################

### A ###

readonly opts="-o gknvV -l version,m4,m4-options:,no-preproc"
...
if [ "$1" = 'test:' ]; then
	...
	case "$1" in
	    ...
	    -c)
		the_re="\<$2\>"
		if [[ ! "$wslTestCommands" =~ ${the_re} ]]; then
